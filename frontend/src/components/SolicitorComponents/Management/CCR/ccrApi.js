...
// Add this new function to ccrApi.js
...
export const generateCCRSubmission = async (data) => {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      console.log('Full headers object:', response.headers);
      console.log(
        'Headers as JSON:',
        JSON.stringify(response.headers, null, 2)
      );

      // Check for specific headers (case-insensitive)
      ...
      console.log(
        'x-ccr-record-count:',
        response.headers['x-ccr-record-count']
      );
      console.log(
        'X-CCR-Record-Count:',
        response.headers['X-CCR-Record-Count']
      ...
      console.log(
        ...
        response.headers['x-ccr-reference-date']
      );
      console.log('x-ccr-test-mode:', response.headers['x-ccr-test-mode']);
      console.log('x-ccr-summary:', response.headers['x-ccr-summary']);

      // List all available header keys
      console.log('Available header keys:', Object.keys(response.headers));
      console.log('=== END HEADERS DEBUG ===');

      // Success - extract headers and return file data
