...
export function EstateSummaryForApp({ estates, requestedAmount, currency_sign }) {
  ...
  const maxAdvanceForAgents = lendableIrishEstate * 0.55;
  ...
  return (
    <div className={`mt-4 p-3 bg-light rounded border ${requested > maxAdvance ? "border-danger":"border-success"}`}>
      <div className="mb-2 pb-2 border-bottom">
        ...
        <div>
          <span className="fw-bold">Maximum Advance (Agents) (55%): </span>
          <span>
            {currency_sign}
            {maxAdvanceForAgents.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </span>
        </div>
      </div>
      <div>
